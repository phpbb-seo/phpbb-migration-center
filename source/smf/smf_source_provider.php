...
use phpbbseo\migrationcenter\source\smf\version\smf_version_detector;

/**
 * SMF 2.x Source Provider
 */
class smf_source_provider implements source_provider_interface
{
	/** @var string */
	...
	{
		return 'Simple Machines Forum (SMF 2.x)';
	}
	...
	public function detect_version(migration_config_dto $config): string
	{
		try
		{
			...
			$info = smf_version_detector::detect($db, $config->source_path);
	...
	public function run_preflight(migration_config_dto $config): preflight_result_dto
	{
		...
		if (!empty($config->source_path))
		{
			...
			{
				$detected = smf_config_detector::detect_from_path($config->source_path);
				if ($detected)
				{
		...
		if ($has_source_path)
		{
			...
			$settings_file = $source_path . '/Settings.php';
			if (file_exists($settings_file) && is_readable($settings_file))
			{
				$result->add_item('source_path', 'Source Root Path & Configuration', 'success', "SMF directory and Settings.php located at: {$source_path}");
			}
			else
			{
				$result->add_item('source_path', 'Source Root Path', 'warning', "SMF directory found, but Settings.php is missing or unreadable.");
			}
		}
		...
		{
			$db = new smf_db_adapter($config);
			$result->add_item('db_connection', 'Database Connection', 'success', "Successfully connected to SMF database: `{$config->db_name}` on {$config->db_host}:{$config->db_port}");
		}
		catch (\Throwable $e)
		{
		...
		// 5. Version Detection
		$version_info = smf_version_detector::detect($db, $config->source_path);
		if (!$version_info['is_supported'])
		{
			$result->add_item('smf_version', 'SMF Version Detection', 'failure', $version_info['error'] ?? 'Unsupported SMF version.');
			return $result;
		}

		$result->add_item('smf_version', 'SMF Version Detection', 'success', "Detected SMF (Version: {$version_info['version_string']}, Major: {$version_info['major_version']}, Confidence: {$version_info['confidence']})");

		// 6. Required Table Fingerprints Validation
		$common_tables = [
			'members', 'membergroups', 'categories', 'boards', 'topics', 'messages',
			'attachments', 'personal_messages', 'pm_recipients', 'polls', 'poll_choices',
			'log_polls', 'ban_groups', 'ban_items', 'settings'
		];
		...
		{
			$result->add_item('required_tables', 'Core Database Tables', 'success', 'All required SMF core tables exist with valid schema.');
		}
		else
		{
			$result->add_item('required_tables', 'Core Database Tables', 'failure', 'Missing required SMF tables: ' . implode(', ', $missing_tables));
		}
		...
		$pk_checks = [
			...
			'topics'            => 'id_topic',
			'messages'          => 'id_msg',
			'attachments'       => 'id_attach',
			'personal_messages' => 'id_pm',
			'polls'             => 'id_poll',
			'ban_groups'        => 'id_ban_group',
			'ban_items'         => 'id_ban',
		];
	...
	public function get_max_source_id(string $step_name, migration_config_dto $config)
	{
		...
		switch ($step_name)
		{
			...
			case 'global_permissions':
				return (int)$db->fetch_one("SELECT MAX(id_group) FROM " . $db->get_table_name('membergroups'));

			case 'users':
			case 'group_memberships':
				return (int)$db->fetch_one("SELECT MAX(id_member) FROM " . $db->get_table_name('members'));

			case 'avatars':
				return (int)$db->fetch_one("SELECT MAX(id_attach) FROM " . $db->get_table_name('attachments') . " WHERE id_member > 0 OR attachment_type = 1");

			case 'bans':
				return (int)$db->fetch_one("SELECT MAX(id_ban) FROM " . $db->get_table_name('ban_items'));

			case 'forums':
			case 'node_permissions':
				return (int)$db->fetch_one("SELECT MAX(id_board) FROM " . $db->get_table_name('boards'));

			case 'topics':
				return (int)$db->fetch_one("SELECT MAX(id_topic) FROM " . $db->get_table_name('topics'));

			case 'posts':
				return (int)$db->fetch_one("SELECT MAX(id_msg) FROM " . $db->get_table_name('messages'));

			case 'attachments':
				return (int)$db->fetch_one("SELECT MAX(id_attach) FROM " . $db->get_table_name('attachments') . " WHERE attachment_type = 0");

			case 'conversations':
			case 'conversation_messages':
				return (int)$db->fetch_one("SELECT MAX(id_pm) FROM " . $db->get_table_name('personal_messages'));

			case 'conversation_attachments':
				return 0;

			case 'polls':
				return (int)$db->fetch_one("SELECT MAX(id_poll) FROM " . $db->get_table_name('polls'));
	...
	public function get_total_records(string $step_name, migration_config_dto $config): int
	{
		...
		switch ($step_name)
		{
			case 'groups':
			case 'global_permissions':
				return (int)$db->fetch_one("SELECT COUNT(*) FROM " . $db->get_table_name('membergroups'));

			case 'users':
			case 'group_memberships':
				return (int)$db->fetch_one("SELECT COUNT(*) FROM " . $db->get_table_name('members'));

			case 'avatars':
				return (int)$db->fetch_one("SELECT COUNT(*) FROM " . $db->get_table_name('attachments') . " WHERE id_member > 0 OR attachment_type = 1");

			case 'bans':
				return (int)$db->fetch_one("SELECT COUNT(*) FROM " . $db->get_table_name('ban_items'));

			case 'forums':
				$c_cnt = (int)$db->fetch_one("SELECT COUNT(*) FROM " . $db->get_table_name('categories'));
				$b_cnt = (int)$db->fetch_one("SELECT COUNT(*) FROM " . $db->get_table_name('boards'));
				return $c_cnt + $b_cnt;

			case 'node_permissions':
				return (int)$db->fetch_one("SELECT COUNT(*) FROM " . $db->get_table_name('boards'));

			case 'topics':
				return (int)$db->fetch_one("SELECT COUNT(*) FROM " . $db->get_table_name('topics'));

			case 'posts':
				return (int)$db->fetch_one("SELECT COUNT(*) FROM " . $db->get_table_name('messages'));

			case 'attachments':
				return (int)$db->fetch_one("SELECT COUNT(*) FROM " . $db->get_table_name('attachments') . " WHERE attachment_type = 0");

			case 'conversations':
			case 'conversation_messages':
				return (int)$db->fetch_one("SELECT COUNT(*) FROM " . $db->get_table_name('personal_messages'));

			case 'conversation_attachments':
				return 0; // NOT APPLICABLE in standard SMF core
	...
	public function get_feature_compatibility(): array
	{
		return [
			'pm_attachments' => [
				'supported' => false,
				'reason'    => 'Native SMF 2.x core does not support attachments in Personal Messages (NOT APPLICABLE).',
			],
			'password_auth' => [
				'supported' => true,
				'reason'    => 'SMF 2.1 Bcrypt and SMF 2.0 SHA1 salted hashing verified. Re-hashed seamlessly upon successful login in phpBB.',
			],
		];
	}
}
