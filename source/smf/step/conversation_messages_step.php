...
/**
 * SMF 2.x Private Messages Delivery & Content Step
 */
class conversation_messages_step implements step_interface
{
	...
	public function process_batch(
		...
		$sql = "SELECT id_pm, id_member_from, msgtime, body
				FROM {$tbl_pm}
		...
		foreach ($rows as $row)
		{
			...
			$dto->user_source_id = (int)($row['id_member_from'] ?? 0);
			$dto->message_date = (int)($row['msgtime'] ?? time());
			$dto->message_text = (string)($row['body'] ?? '');
