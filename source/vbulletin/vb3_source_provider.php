...
/**
 * vBulletin 3.8 Dedicated Source Provider
 */
class vb3_source_provider extends vbulletin_source_provider
{
	/**
	...
	{
		return 'vBulletin 3.8.x';
	}

	/**
	 * Run preflight checks with vB3 specific validation
	 *
	...
	public function run_preflight(migration_config_dto $config): preflight_result_dto
	{
		...
		try
		{
			...
			if ((int)($info['major_version'] ?? 0) === 4)
			{
				$result->add_item('vb_version_mismatch', 'vBulletin Target Version', 'warning', "Selected platform is vBulletin 3.8, but detected database version is {$info['version_string']} (vBulletin 4.x). Migration Center will adapt to vBulletin 4 schema automatically.");
			}
		}
		catch (\Throwable $e)
		{
