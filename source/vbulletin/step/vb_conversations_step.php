...
class vb_conversations_step implements step_interface
{
	...
	public function process_batch(
		...
		if ($has_vb6_pm)
		{
			...
			$tbl_sentto = $db->get_table_name('sentto');

			$sql = "SELECT n.nodeid, n.userid AS fromuserid, n.authorname AS fromusername, n.title, n.created AS dateline,
			               n.lastcontent AS last_message_date, n.lastcontentid AS last_message_id
					FROM {$tbl_node} n
					WHERE n.contenttypeid = 27 AND n.starter = n.nodeid AND n.nodeid > {$cursor_id}
			...
			foreach ($rows as $row)
			{
				...
				$dto->first_message_id = $nodeid;
				$dto->last_message_id = (int)($row['last_message_id'] ?: $nodeid);
				$dto->last_message_date = (int)($row['last_message_date'] ?: $dto->start_date);

				$recipients = [];
				if ($db->table_exists('sentto'))
				{
					$sent_rows = $db->fetch_all("SELECT userid, msgread FROM {$tbl_sentto} WHERE nodeid = {$nodeid}");
					foreach ($sent_rows as $sr)
					{
						$uid = (int)$sr['userid'];
						$r_dto = new conversation_recipient_dto();
						$r_dto->user_source_id = $uid;
						$r_dto->recipient_state = 'active';
						$r_dto->last_read_date = !empty($sr['msgread']) ? $dto->start_date : 0;
						$r_dto->is_unread = empty($sr['msgread']);
						$r_dto->join_date = $dto->start_date;
						$recipients[$uid] = $r_dto;
					}
				}
