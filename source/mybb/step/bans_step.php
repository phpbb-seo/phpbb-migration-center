...
class bans_step implements step_interface
{
	...
	public function process_batch(
		...
		$tbl_ban = $db->get_table_name('banned');
		...
		$sql = "SELECT uid, gid, admin, dateline, lifted, reason
				FROM {$tbl_ban}
				WHERE uid > {$cursor_id}
				ORDER BY uid ASC
				LIMIT {$batch_size}";
		...
		foreach ($rows as $row)
		{
			...
			$dto->ban_start = (int)($row['dateline'] ?? time());
			$dto->ban_end = (int)($row['lifted'] ?? 0);
			$dto->ban_reason = trim((string)($row['reason'] ?? 'Imported MyBB ban'));
